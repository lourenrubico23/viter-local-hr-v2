...
const DirectReportTable = ({ setItemEdit, employees }) => {
  ...
  const {
    ...
    queryKey: ["direct-report", onSearch, store.isSearch, isFilter, statusData],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/v2/direct-report/search`, // search endpoint
        `/v2/direct-report/page/${pageParam}`, // list endpoint
        store.isSearch || isFilter, // search boolean
        {
          searchValue: search.current.value,
          id: "",
          isFilter,
          direct_report_is_active: statusData === "all" ? "" : statusData,
        } // search value
      ),
    ...
    <>
      ...
      {store.isArchive && (
        <ModalArchive
          ...
          mysqlEndpoint={`/v2/direct-report/active/${id}`}
          item={isData}
          archive={isArchiving}
        />
      )}
      {store.isDelete && (
        <ModalDelete
          ...
          queryKey={"direct-report"}
          mysqlEndpoint={`/v2/direct-report/${id}`}
          item={isData}
        />
      )}
      {store.isRestore && (
