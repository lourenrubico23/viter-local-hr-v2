...
const FbsAdminTable = ({ setItemEdit }) => {
  ...
  const {
    ...
    queryKey: ["admin", onSearch, store.isSearch],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/v2/admin/search`, // search endpoint
        `/v2/admin/page/${pageParam}`, // list endpoint
    ...
    <>
              ...
              <React.Fragment key={key}>
                {page.data.map((item, key) => (
                  <tr key={key}>
                    ...
                    <td>
                      {item.user_admin_fname} {item.user_admin_lname}
                    </td>
                    <td>{item.user_admin_email}</td>
                    <td className="flex items-center gap-3 justify-end mt-2 lg:mt-0">
                      {item.user_admin_is_active ? (
                        <>
                          <button
      ...
      {store.isArchive && (
        <ModalArchive
          ...
          queryKey={"admin"}
          mysqlEndpoint={`/v2/admin/active/${id}`}
          item={isData}
          archive={isArchiving}
        />
      )}
